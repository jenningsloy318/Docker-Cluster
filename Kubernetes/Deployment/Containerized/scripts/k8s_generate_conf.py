...
def create_single_etcd_conf(hostIP, etcd_dir_root, conf_file):
    ...
    etcd_conf_content.write("data-dir: %s\n" % etcd_datadir)
    etcd_conf_content.write(
        "initial-cluster: default=http://%s:2380\n" % hostIP)
    etcd_conf_content.write(
        "initial-advertise-peer-urls: http://%s:2380\n" % hostIP)
    etcd_conf_content.write("initial-cluster-token: etcd-cluster\n")
    etcd_conf_content.write("initial-cluster-state: new\n")
...
def create_file_from_template(values_dict, Template_file_basedir, Template_file_name, Target_file_basedir, Target_file_name):
    ...
    Target_file_template_env = Environment(
        loader=FileSystemLoader(Template_file_basedir))
    Target_file_tepmlate = Target_file_template_env.get_template(
        Template_file_name)
...
if __name__ == '__main__':
    ...
    arguments.add_argument("-c", "--config", nargs="?",
                           help="k8s deployment conf file", required=True)
    if len(sys.argv) == 1:
...
# process the config file
    ...
    K8S_ADDONS_DIR=K8S_DIR_ROOT + 'addons/'
    ...
    TEMP_LOCAL_ETCD_CONF_DIR = TEMP_LOCAT_ROOT + \
        ('/').join(ETCD_CONF_DIR.split('/')[1:])
    TEMP_LOCAL_ETCD_DATA_DIR = TEMP_LOCAT_ROOT + \
        ('/').join(ETCD_DATA_DIR.split('/')[1:])

    TEMP_LOCAL_K8S_CONF_DIR = TEMP_LOCAT_ROOT + \
        ('/').join(K8S_CONF_DIR.split('/')[1:])
    TEMP_LOCAL_K8S_CERT_DIR = TEMP_LOCAT_ROOT + \
        ('/').join(K8S_CERT_DIR.split('/')[1:])
    ...
    TEMP_LOCAL_K8S_ADDONS_DIR = TEMP_LOCAT_ROOT + ('/').join(K8S_ADDONS_DIR.split('/')[1:])
...
# extract worker nodes
    ...
    ALL_NODES_ADDR=ALL_MASTER_NODES_ADDR+WORKER_NODES_ADDR
# define etcd endpoints
    if DEPLOY_MODE == 'multiple':
        for index, host in enumerate(MASTER_NODES_ADDR):
            etcd_cluster_peer_endpoints.append(
                ''.join(['etcd', str(index), '=http://', host, ':2380']))
            etcd_cluster_client_endpoints.append(
                ''.join(['http://', host, ':2379']))

    else:
        etcd_cluster_peer_endpoints = ''.join(
            ['default', '=http://', MASTER_NODES_ADDR[0], ':2380'])
        etcd_cluster_client_endpoints = ''.join(
            ['http://', MASTER_NODES_ADDR[0], ':2379'])
    ...
    if DEPLOY_MODE == 'multiple':
        ...
        for index, host in enumerate(MASTER_NODES_ADDR):
            ...
            etcd_cluster_conf = TEMP_LOCAT_ROOT + '/' + \
                host + '/' + ETCD_CONF_DIR + '/etcd.conf'
            etcd_cluster_conf_path = Path(
                ('/').join(etcd_cluster_conf.split('/')[:-1]))
            etcd_cluster_node_name = 'etcd' + str(index)
            if not etcd_cluster_conf_path.exists():
                etcd_cluster_conf_path.mkdir(parents=True)
            create_etcd_cluster_conf(
                host, etcd_cluster_node_name, ETCD_DIR_ROOT, etcd_cluster_peer_endpoints, etcd_cluster_conf)
    else:
...
# create openssl.conf in certs folder
    if MASTER_NODES_ADDR or MASTER_NODES_DNS:
        ...
        APISERVERS_DNSs = ['kubernetes', 'kubernetes.default',
                           'kubernetes.default.svc', 'kubernetes.default.svc.' + CLUSTER_DOMAIN]

        APISERVERS_IPs.extend(MASTER_NODES_ADDR)
        APISERVERS_DNSs.extend(MASTER_NODES_DNS)

        openssl_conf_values_dict = {
            'IPs': APISERVERS_IPs, 'DNSs': APISERVERS_DNSs}
        ...
        create_file_from_template(
            openssl_conf_values_dict, Template_dir, Template_file, Target_dir, Target_file)

    if WORKER_NODES_ADDR or WORKER_NODES_DNS:
        print('Starting to generate openssl conf  for signing worker nodes.\n')

        openssl_conf_values_dict = {
            'IPs': WORKER_NODES_ADDR, 'DNSs': WORKER_NODES_DNS}
...
# modify k8s pod files
    ...
    apiserver_dict = {'SVC_CIDR': SVC_CIDR, 'K8S_VER': K8S_VER，'K8S_DIR_ROOT':K8S_DIR_ROOT}
    create_file_from_template(
        apiserver_dict, Template_dir, Template_file, Target_dir, Target_file)
    ...
    controller_manager_dict = {'POD_CIDR': POD_CIDR,
                               'K8S_VER': K8S_VER, 'KUBECONFIG': KUBECONFIG,'K8S_DIR_ROOT':K8S_DIR_ROOT}
    create_file_from_template(
        controller_manager_dict, Template_dir, Template_file, Target_dir, Target_file)
    ...
    KUBECONFIG = K8S_ADMIN_CONF
    scheduler_dict = {'POD_CIDR': POD_CIDR,
                      'K8S_VER': K8S_VER, 'KUBECONFIG': KUBECONFIG,'K8S_DIR_ROOT':K8S_DIR_ROOT}
    create_file_from_template(
        scheduler_dict, Template_dir, Template_file, Target_dir, Target_file)
    ...
    ETCDE_CONF = ETCD_CONF_DIR + 'etcd.conf'
    etcd_dict = {'ETCD_VER': ETCD_VER, 'ETCDE_CONF': ETCDE_CONF,'ETCD_DIR_ROOT':ETCD_DIR_ROOT}
    create_file_from_template(etcd_dict, Template_dir,
                              Template_file, Target_dir, Target_file)

    # creating kube-proxy.yaml
    print('Generating kube-proxy.yaml.\n')
    Template_dir = './templates/addons/'
    Template_file = 'kube-proxy.yaml.jinja2'
    Target_dir = TEMP_LOCAL_K8S_ADDONS_DIR
    Target_file = 'kube-proxy.yaml'
    KUBECONFIG = K8S_KUBE_PROXY_CONF
    proxy_dict = {'POD_CIDR': POD_CIDR, 'KUBECONFIG': KUBECONFIG,'K8S_DIR_ROOT':K8S_DIR_ROOT}
    create_file_from_template(proxy_dict, Template_dir,
                              Template_file, Target_dir, Target_file)
    ...
    calico_dict = {
        'ETCD_CLUSTER_CLIENT_ENDPOINTS': ETCD_CLUSTER_CLIENT_ENDPOINTS, 'POD_CIDR': POD_CIDR}
    create_file_from_template(
        calico_dict, Template_dir, Template_file, Target_dir, Target_file)
    ...
    create_file_from_template(
        kube_dns_dict, Template_dir, Template_file, Target_dir, Target_file)
...
# modify kubelet.service
    ...
    kubelet_service_dict = {'CLUSTER_DNS': CLUSTER_DNS,
                            'CLUSTER_DOMAIN': CLUSTER_DOMAIN, 'KUBECONFIG': KUBECONFIG}
    create_file_from_template(
        kubelet_service_dict, Template_dir, Template_file, Target_dir, Target_file)
    ...
    docker_service_dict = {'DOCKER_ENGINE_PROXY': DOCKER_ENGINE_PROXY,
                           'DOCKER_ENGINE_NOPROXY': DOCKER_ENGINE_NOPROXY}
    create_file_from_template(
        docker_service_dict, Template_dir, Template_file, Target_dir, Target_file)
...
# modify shell scripts
    ...
    create_file_from_template(shell_get_k8s_server_bin_dict,
                              Template_dir, Template_file, Target_dir, Target_file)
    ...
    create_file_from_template(shell_install_docker_engine_dict,
                              Template_dir, Template_file, Target_dir, Target_file)
    ...
    if DEPLOY_MODE == 'multiple':
        ...
        create_file_from_template(
            nginx_conf_dict, Template_dir, Template_file, Target_dir, Target_file)
        ...
        Target_file = 'apiserver-proxy.conf'
        apiserver_proxy_conf_template = {'MASTER_HOSTS':MASTER_NODES_ADDR}
        create_file_from_template(
            apiserver_proxy_conf_template, Template_dir, Template_file, Target_dir, Target_file)
    ...
    if CREATE_SELF_CA:
        ...
        print(bytes.decode(subprocess.Popen(CMD_CREATE_CA_KEY, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_DIR).communicate()[0]))
        ...
        print(bytes.decode(subprocess.Popen(CMD_CREATE_SELF_CA_CERT, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_DIR).communicate()[0]))

        print('creating apiserver priviate key in %s.\n' %
              TEMP_LOCAL_K8S_CERT_MASTER_DIR)
        CMD_CREATE_APISERVER_KEY = 'openssl genrsa -out apiserver-key.pem 2048'
        print(bytes.decode(subprocess.Popen(CMD_CREATE_APISERVER_KEY, shell=True, stdout=subprocess.PIPE,
        ...
        print(bytes.decode(subprocess.Popen(CMD_CREATE_APISERVER_SIGN_REQ, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_MASTER_DIR).communicate()[0]))
        ...
        print(bytes.decode(subprocess.Popen(CMD_CREATE_APISERVER_CERT, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_MASTER_DIR).communicate()[0]))
        ...
        print(bytes.decode(subprocess.Popen(CMD_CREATE_ADMIN_KEY, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_MASTER_DIR).communicate()[0]))
        ...
        print(bytes.decode(subprocess.Popen(CMD_CREATE_BASIC_AUTH, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_MASTER_DIR).communicate()[0]))
        ...
        print(bytes.decode(subprocess.Popen(CMD_CREATE_TOKEN_AUTH, shell=True, stdout=subprocess.PIPE,
                                            stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_MASTER_DIR).communicate()[0]))
        ...
        if WORKER_NODES_ADDR or WORKER_NODES_DNS:
            ...
            print(bytes.decode(subprocess.Popen(CMD_CREATE_WORKER_KEY, shell=True, stdout=subprocess.PIPE,
            ...
            print(bytes.decode(subprocess.Popen(CMD_CREATE_WORKER_CERT, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CERT_WORKER_DIR).communicate()[0]))
    ...
    if DEPLOY_MODE == 'multiple':
        ...
        MULTIPLE_APISERVER_ENDPOINT = 'https://' + \
            ALL_MASTER_ITEMS['loadbalancer_vip_addr'] + ':6443'

        CMD_CREATE_ADMIN_KUBECONFIG_1 = 'kubectl config set-cluster devosp-k8s --certificate-authority=' + \
            K8S_CA_CERT + ' --server=' + MULTIPLE_APISERVER_ENDPOINT + \
            ' --kubeconfig=' + ADMIN_CONF
        CMD_CREATE_ADMIN_KUBECONFIG_2 = 'kubectl config set-credentials admin  --client-certificate=' + \
            K8S_ADMIN_CERT + ' --client-key=' + K8S_ADMIN_KEY + ' --kubeconfig=' + ADMIN_CONF
        ...
        for CMD in [CMD_CREATE_ADMIN_KUBECONFIG_1, CMD_CREATE_ADMIN_KUBECONFIG_2, CMD_CREATE_ADMIN_KUBECONFIG_3, CMD_CREATE_ADMIN_KUBECONFIG_4]:
            print(bytes.decode(subprocess.Popen(CMD, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAL_K8S_CONF_DIR).communicate()[0]))

        print('Generating  kubeconfig for kubelet: kubelet.conf in %s.\n' %
              TEMP_LOCAL_K8S_CONF_DIR)
        CMD_CREATE_KUBELET_KUBECONFIG_1 = 'kubectl config set-cluster devosp-k8s --certificate-authority=' + \
            K8S_CA_CERT + ' --server=' + MULTIPLE_APISERVER_ENDPOINT + \
            ' --kubeconfig=' + KUBELET_CONF
        CMD_CREATE_KUBELET_KUBECONFIG_2 = 'kubectl config set-credentials kubelet  --client-certificate=' + \
            K8S_KUBELET_CERT + ' --client-key=' + \
            K8S_KUBELET_KEY + ' --kubeconfig=' + KUBELET_CONF
        ...
        print('Generating  kubeconfig for kube-proxy: kube-proxy.conf in %s.\n' %
              TEMP_LOCAL_K8S_CONF_DIR)
        CMD_CREATE_KUBE_PROXY_KUBECONFIG_1 = 'kubectl config set-cluster devosp-k8s --certificate-authority=' + \
            K8S_CA_CERT + ' --server=' + MULTIPLE_APISERVER_ENDPOINT + \
            ' --kubeconfig=' + KUBE_PROXY_CONF
        CMD_CREATE_KUBE_PROXY_KUBECONFIG_2 = 'kubectl config set-credentials kube-proxy  --client-certificate=' + \
            K8S_KUBE_PROXY_CERT + ' --client-key=' + \
            K8S_KUBE_PROXY_KEY + ' --kubeconfig=' + KUBE_PROXY_CONF
        ...
        if not Path(TEMP_LOCAL_K8S_CONF_DIR).exists():
            Path(TEMP_LOCAL_K8S_CONF_DIR).mkdir(parents=True)
        SINGLE_APISERVER_ENDPOINT = 'https://' + \
            ALL_MASTER_NODES_ADDR[0] + ':6443'

        print('Generating  kubeconfig for admin: admin.conf in %s.\n' %
              TEMP_LOCAL_K8S_CONF_DIR)

        CMD_CREATE_ADMIN_KUBECONFIG_1 = 'kubectl config set-cluster devosp-k8s --certificate-authority=' + \
            K8S_CA_CERT + ' --server=' + SINGLE_APISERVER_ENDPOINT + \
            ' --kubeconfig=' + ADMIN_CONF
        CMD_CREATE_ADMIN_KUBECONFIG_2 = 'kubectl config set-credentials admin  --client-certificate=' + \
            K8S_ADMIN_CERT + ' --client-key=' + K8S_ADMIN_KEY + ' --kubeconfig=' + ADMIN_CONF
        CMD_CREATE_ADMIN_KUBECONFIG_3 = 'kubectl config set-context admin@devosp-k8s --cluster=devosp-k8s --user=admin  --kubeconfig=' + ADMIN_CONF
    ...
    print('Files are generated, begin to copy them to proper directories\n')
    print('Begin to process master node files\n')
    if DEPLOY_MODE == 'multiple':
        for MASTER in MASTER_NODES_ADDR:
            TEMP_LOCAL_K8S_CERT_FINAL_DIR = MASTER + K8S_CERT_DIR
            TEMP_LOCAL_K8S_CONF_FINAL_DIR = MASTER + K8S_CONF_DIR
            TEMP_LOCAL_K8S_MANIFESTS_FINAL_DIR = MASTER + K8S_MANIFESTS_DIR
            TEMP_LOCAL_K8S_ADDONS_FINAL_DIR = MASTER + K8S_ADDONS_DIR
            TEMP_LOCAL_K8S_SHELL_FINAL_DIR =  MASTER + K8S_SHELL_DIR
            TEMP_LOCAL_K8S_INIT_FINAL_DIR =  MASTER + '/lib/systemd/system/'
            for DIR in [TEMP_LOCAL_K8S_CERT_FINAL_DIR, TEMP_LOCAL_K8S_CONF_FINAL_DIR, TEMP_LOCAL_K8S_MANIFESTS_FINAL_DIR, TEMP_LOCAL_K8S_SHELL_FINAL_DIR, TEMP_LOCAL_K8S_INIT_FINAL_DIR,TEMP_LOCAL_K8S_ADDONS_FINAL_DIR]:
                CMD_CREATE_DIR='mkdir -p '+DIR
                print(bytes.decode(subprocess.Popen(CMD_CREATE_DIR, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))

            CMD_COPY_CERT = 'cp data/kubernetes/certs/ca.pem data/kubernetes/certs/ca-key.pem data/kubernetes/certs/master/*.pem ' + \
                TEMP_LOCAL_K8S_CERT_FINAL_DIR
            CMD_COPY_CONF = 'cp data/kubernetes/conf/* ' + TEMP_LOCAL_K8S_CONF_FINAL_DIR
            CMD_COPY_MANIFESTS = 'cp data/kubernetes/manifests/* ' + TEMP_LOCAL_K8S_MANIFESTS_FINAL_DIR
            CMD_COPY_ADDONS = 'cp data/kubernetes/manifests/* ' + TEMP_LOCAL_K8S_ADDONS_FINAL_DIR
            CMD_COPY_SHELL = 'cp data/kubernetes/shell/* ' + TEMP_LOCAL_K8S_SHELL_FINAL_DIR
            CMD_COPY_INIT = 'cp -r lib/systemd/system/* ' + TEMP_LOCAL_K8S_INIT_FINAL_DIR
            for CMD in [CMD_COPY_CERT,CMD_COPY_CONF,CMD_COPY_MANIFESTS,CMD_COPY_ADDONS,CMD_COPY_SHELL,CMD_COPY_INIT]:
              print(bytes.decode(subprocess.Popen(CMD, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))
            for LBNODE in LB_NODES_ADDR:
              TEMP_LOCAL_K8S_ETC_FINAL_DIR=  LBNODE+'/etc/ningx'
              CMD_CREATE_DIR='mkdir -p '+TEMP_LOCAL_K8S_ETC_FINAL_DIR
              print(bytes.decode(subprocess.Popen(CMD_CREATE_DIR, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))

              CMD_COPY_ETC='cp -r etc/ningx/* '+TEMP_LOCAL_K8S_ETC_FINAL_DIR
              print(bytes.decode(subprocess.Popen(CMD_COPY_ETC, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))

            if WORKER_NODES_ADDR or WORKER_NODES_DNS:
              for WORKER in MASTER_NODES_ADDR:
                TEMP_LOCAL_K8S_CERT_FINAL_DIR =  WORKER + K8S_CERT_DIR
                TEMP_LOCAL_K8S_CONF_FINAL_DIR =  WORKER + K8S_CONF_DIR
                TEMP_LOCAL_K8S_SHELL_FINAL_DIR =  WORKER + K8S_SHELL_DIR
                TEMP_LOCAL_K8S_INIT_FINAL_DIR = WORKER + '/lib/systemd/system/'
                for DIR in [TEMP_LOCAL_K8S_CERT_FINAL_DIR, TEMP_LOCAL_K8S_CONF_FINAL_DIR, TEMP_LOCAL_K8S_MANIFESTS_FINAL_DIR, TEMP_LOCAL_K8S_SHELL_FINAL_DIR, TEMP_LOCAL_K8S_INIT_FINAL_DIR]:
                  CMD_CREATE_DIR='mkdir -p '+DIR
                  print(bytes.decode(subprocess.Popen(CMD_CREATE_DIR, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))
                CMD_COPY_CERT = 'cp data/kubernetes/certs/ca.pem data/kubernetes/certs/ca-key.pem data/kubernetes/certs/worker/*.pem ' + \
                TEMP_LOCAL_K8S_CERT_FINAL_DIR
                CMD_COPY_CONF = 'cp data/kubernetes/conf/* ' + TEMP_LOCAL_K8S_CONF_FINAL_DIR
                CMD_COPY_SHELL = 'cp data/kubernetes/shell/* ' + TEMP_LOCAL_K8S_SHELL_FINAL_DIR
                CMD_COPY_INIT = 'cp -r lib/systemd/system/* ' + TEMP_LOCAL_K8S_INIT_FINAL_DIR
                for CMD in [CMD_COPY_CERT,CMD_COPY_CONF,CMD_COPY_SHELL,CMD_COPY_INIT]:
                  print(bytes.decode(subprocess.Popen(CMD, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))
    else:
       MASTER=MASTER_NODES_ADDR[0]
       TEMP_LOCAL_K8S_CERT_FINAL_DIR =  MASTER + K8S_CERT_DIR
       TEMP_LOCAL_K8S_CONF_FINAL_DIR =  MASTER + K8S_CONF_DIR
       TEMP_LOCAL_K8S_MANIFESTS_FINAL_DIR =  MASTER + K8S_MANIFESTS_DIR
       TEMP_LOCAL_K8S_ADDONS_FINAL_DIR = MASTER + K8S_ADDONS_DIR
       ...
       CMD_COPY_SHELL = 'cp data/kubernetes/shell/* ' + TEMP_LOCAL_K8S_SHELL_FINAL_DIR
       CMD_COPY_INIT = 'cp -r lib/systemd/system/* ' + TEMP_LOCAL_K8S_INIT_FINAL_DIR
       for CMD in [CMD_COPY_CERT,CMD_COPY_CONF,CMD_COPY_MANIFESTS,CMD_COPY_SHELL,CMD_COPY_INIT]:
        print(bytes.decode(subprocess.Popen(CMD, shell=True, stdout=subprocess.PIPE,
                                           stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))
       if WORKER_NODES_ADDR or WORKER_NODES_DNS:
         for WORKER in MASTER_NODES_ADDR:
             TEMP_LOCAL_K8S_CERT_FINAL_DIR = WORKER + K8S_CERT_DIR
             TEMP_LOCAL_K8S_CONF_FINAL_DIR =  WORKER + K8S_CONF_DIR
             TEMP_LOCAL_K8S_MANIFESTS_FINAL_DIR =  WORKER + K8S_MANIFESTS_DIR
             TEMP_LOCAL_K8S_ADDONS_FINAL_DIR = MASTER + K8S_ADDONS_DIR
             TEMP_LOCAL_K8S_SHELL_FINAL_DIR =  WORKER + K8S_SHELL_DIR
             TEMP_LOCAL_K8S_INIT_FINAL_DIR =  WORKER + '/lib/systemd/system/'
             for DIR in [TEMP_LOCAL_K8S_CERT_FINAL_DIR, TEMP_LOCAL_K8S_CONF_FINAL_DIR, TEMP_LOCAL_K8S_MANIFESTS_FINAL_DIR,TEMP_LOCAL_K8S_ADDONS_FINAL_DIR, TEMP_LOCAL_K8S_SHELL_FINAL_DIR, TEMP_LOCAL_K8S_INIT_FINAL_DIR]:
              CMD_CREATE_DIR='mkdir -p '+DIR
              print(bytes.decode(subprocess.Popen(CMD_CREATE_DIR, shell=True, stdout=subprocess.PIPE,
                                                stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))
             CMD_COPY_CERT = 'cp data/kubernetes/certs/ca.pem data/kubernetes/certs/ca-key.pem data/kubernetes/certs/worker/*.pem '+TEMP_LOCAL_K8S_CERT_FINAL_DIR
             CMD_COPY_CONF = 'cp data/kubernetes/conf/* ' + TEMP_LOCAL_K8S_CONF_FINAL_DIR
             CMD_COPY_SHELL = 'cp data/kubernetes/shell/* ' + TEMP_LOCAL_K8S_SHELL_FINAL_DIR
             CMD_COPY_INIT = 'cp -r lib/systemd/system/* ' + TEMP_LOCAL_K8S_INIT_FINAL_DIR
             for CMD in [CMD_COPY_CERT,CMD_COPY_CONF,CMD_COPY_SHELL,CMD_COPY_INIT]:
    ...
    for NODE in ALL_NODES_ADDR:
      print('Copy files to remote node %s.\n'%NODE)

      CMD_RSYNC='rsync -av --progress '+NODE+'/ '+NODE+":/"
      print(bytes.decode(subprocess.Popen(CMD_RSYNC, shell=True, stdout=subprocess.PIPE,
                                                 stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))

      print('Installing on node %s.\n'%NODE)

      CMD_INSTALL_DOCKER_ENGINE='ssh '+NODE+' "'+K8S_SHELL_DIR+'k8s_install_docker_engine.sh"'
      print(bytes.decode(subprocess.Popen(CMD_INSTALL_DOCKER_ENGINE, shell=True, stdout=subprocess.PIPE,
                                                 stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))
      CMD_INSTALL_K8S_CLIENT='ssh '+NODE+' "'+K8S_SHELL_DIR+'get_k8s_server_bin.sh"'
      print(bytes.decode(subprocess.Popen(CMD_INSTALL_K8S_CLIENT, shell=True, stdout=subprocess.PIPE,
                                                 stderr=subprocess.STDOUT, cwd=TEMP_LOCAT_ROOT).communicate()[0]))
